...
//AddSuccessor call to
//		add each one to the current search - return false if you are out
//of memory and the search
//		will fail
//
...
class PuzzleState {
   ...
   private:
    // User stuff - Just add what you need to help you write the above
    // functions...

    void GetSpacePosition(PuzzleState* pn, int* rx, int* ry);
    bool LegalMove(TILE* StartTiles,
                   TILE* TargetTiles,
                   int   spx,
                   int   spy,
                   int   tx,
...
// Three example start states from Bratko's Prolog Programming for Artificial
// Intelligence

#if 1
    // ex a - 4 steps
    TL_1, TL_3, TL_4, TL_8, TL_SPACE, TL_2, TL_7, TL_6, TL_5,
...
void PuzzleState::PrintNodeInfo() {
    char str[100];
    sprintf(str, "%c %c %c\n%c %c %c\n%c %c %c\n", tiles[0] + '0',
            tiles[1] + '0', tiles[2] + '0', tiles[3] + '0', tiles[4] + '0',
            tiles[5] + '0', tiles[6] + '0', tiles[7] + '0', tiles[8] + '0');
...
float PuzzleState::GoalDistanceEstimate(PuzzleState& nodeGoal) {
    ...
    // given a table index returns the index of the tile that is clockwise to it
    // 3*3 only
...
// Given a node set of tiles and a set of tiles to move them into, do the move
// as if it was on a tile board
// note : returns false if the board wasn't changed, and simply returns the
// tiles as they were in the target
// spx and spy is the space position while tx and ty is the target move from
// position

bool PuzzleState::LegalMove(TILE* StartTiles,
                            TILE* TargetTiles,
                            int   spx,
                            int   spy,
                            int   tx,
    ...
    if (GetMap(spx, spy, StartTiles) == GM_SPACE) {
        if (GetMap(tx, ty, StartTiles) == GM_TILE) {
            ...
            TargetTiles[(ty * BOARD_WIDTH) + tx] =
                StartTiles[(spy * BOARD_WIDTH) + spx];
            TargetTiles[(spy * BOARD_WIDTH) + spx] =
                StartTiles[(ty * BOARD_WIDTH) + tx];
...
// This generates the successors to the given PuzzleState. It uses a helper
// function called
...
bool PuzzleState::GetSuccessors(AStarSearch<PuzzleState>* astarsearch,
                                PuzzleState*              parent_node) {
    PuzzleState NewNode;
...
// Test cancel search
            ...
            SearchSteps++;
                 ...
                 AStarSearch<PuzzleState>::SEARCH_STATE_SEARCHING);
        ...
        if (SearchState == AStarSearch<PuzzleState>::SEARCH_STATE_SUCCEEDED) {
            ...
            astarsearch.FreeSolutionNodes();
                   ...
                   AStarSearch<PuzzleState>::SEARCH_STATE_FAILED) {
