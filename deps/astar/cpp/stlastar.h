...
class AStarSearch {
   ...
   public:
    class Node {
       public:
        ...
        float
            f;  // sum of cumulative cost of predecessors and self and heuristic
    ...
    unsigned int SearchStep() {
        ...
        } else  // not goal
        {
            ...
            for (typename vector<Node*>::iterator successor =
                ...
                float newg =
                    n->g + n->m_UserState.GetCost((*successor)->m_UserState);

                // Now we need to find whether the node is on the open or closed
                // lists
                // If it is but the node that is already on them is better
                // (lower g)
                ...
                if (openlist_result != m_OpenList.end()) {
                    ...
                    // explaining
                    // it in detail. sort_heap called on an invalid heap does
                    // not work
   ...
   private:  // methods
    ...
    // nodes may be
    // created that are still present when the search ends. They will be deleted
    // by this
