...
void compare_libraries(std::size_t         data_size,
    ...
    Point target(
        0.0);  // target in the center for some consistency in the results
    ...
    }
    {
        ...
        utils::time_point start = utils::process_timer_now();
        for (spatial::neighbor_iterator<spatial::point_multiset<N, Point> > i =
                 spatial::neighbor_begin(cobaye, target);
