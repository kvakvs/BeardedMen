...
void compare_libraries(std::size_t         data_size,
        ...
        data.push_back(Point(distribution));
    {
        ...
        for (spatial::ordered_iterator<spatial::idle_point_multiset<0, Point> >
                 i = ordered_begin(cobaye);
             i != ordered_end(cobaye); ++i)
            ;
        ...
        spatial::ordered_iterator<spatial::idle_point_multiset<0, Point> >
            i   = ordered_end(cobaye),
            end = ordered_begin(cobaye);
        for (; i != end; --i)
            ;
    ...
    }
    {
        ...
        utils::time_point start = utils::process_timer_now();
        for (spatial::ordered_iterator<spatial::point_multiset<0, Point> > i =
...
int main(int argc, char** argv) {
    ...
    utils::random_engine engine(
        17489382);  // constant seed for repeatable tests
