...
BOOST_AUTO_TEST_CASE(text_except_check_dimension) {
    ...
    BOOST_CHECK_NO_THROW(except::check_dimension(fix.container.dimension(), 0));
    BOOST_CHECK_THROW(except::check_dimension(fix.container.dimension(), 2),
                      invalid_dimension);
}
...
BOOST_AUTO_TEST_CASE(text_except_check_node) {
    {
        ...
        BOOST_CHECK_THROW(except::check_node(ptr), invalid_node);
    }
    {
...
BOOST_AUTO_TEST_CASE(text_except_check_iterator) {
    ...
    }
    {
        ...
        details::Node_iterator<details::Kdtree_link<const int2, const int2> > i(
            fix.container.begin().node);
        details::Node_iterator<details::Kdtree_link<const int2, const int2> > j(
        ...
        BOOST_CHECK_THROW(except::check_iterator(i, j), invalid_iterator);
    }
}
...
BOOST_AUTO_TEST_CASE(text_except_check_range) {
    pointset_fix<int2> fix(0);
    BOOST_CHECK_NO_THROW(except::check_open_bounds(fix.container, zeros, ones));
    BOOST_CHECK_NO_THROW(except::check_bounds(fix.container, zeros, ones));
    BOOST_CHECK_NO_THROW(
        except::check_closed_bounds(fix.container, zeros, zeros));
    BOOST_CHECK_THROW(except::check_open_bounds(fix.container, zeros, zeros),
                      invalid_bounds);
    BOOST_CHECK_THROW(except::check_bounds(fix.container, zeros, zeros),
                      invalid_bounds);
    BOOST_CHECK_THROW(except::check_closed_bounds(fix.container, ones, zeros),
                      invalid_bounds);
...
BOOST_AUTO_TEST_CASE(text_except_check_math) {
    ...
    BOOST_CHECK_THROW(
        except::check_positive_add(std::numeric_limits<int>::max(), 1),
        arithmetic_error);
    ...
    BOOST_CHECK_THROW(
        except::check_positive_mul(std::numeric_limits<int>::max(), 2),
        arithmetic_error);
}
