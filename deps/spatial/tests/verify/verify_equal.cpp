...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_equal_basics, Tp, every_quad) {
    ...
    equal_iterator_pair<const typename Tp::container_type> r, s(c, c), t(p);
    BOOST_CHECK(a == b);
    BOOST_CHECK(!(a != c));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_equal_deference, Tp, double6_maps) {
    ...
    double6 model;
    std::fill(model.begin(), model.end(), 1.0);
    equal_iterator<typename Tp::container_type> a(
        fix.container, model, 0, fix.container.end().node->parent);
    equal_iterator<const typename Tp::container_type> b(
        fix.container, model, 0, fix.container.end().node->parent);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_equal_minimum, Tp, double6_sets) {
    {
        ...
        while (!fix.container.empty()) {
            // Randomly pick one of the values in the tree and attempt to find
            // it.
            typename Tp::container_type::iterator pick = fix.container.begin();
            std::advance(pick, (ptrdiff_t)((size_type)std::rand() %
                                           fix.container.size()));
            equal_iterator<typename Tp::container_type> it =
                equal_begin(fix.container, *pick);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_equal_increment, Tp, quad_sets) {
    {
        ...
        while (!fix.container.empty()) {
            ...
            equal_iterator<typename Tp::container_type> eq =
                equal_begin(fix.container, *pick);
    ...
    {  // test at the limit: a tree with 1 element with point-box
        Tp                                                fix(1, same());
        quad                                              model(1, 1, 1, 1);
        equal_iterator<const typename Tp::container_type> i = equal_begin(
                                                              fix.container,
                                                              model),
                                                          j = i;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_equal_decrement, Tp, quad_sets) {
    ...
    {  // test at the limit: a tree with 1 element with point-box
        Tp                                          fix(1, same());
        quad                                        model(1, 1, 1, 1);
        equal_iterator<typename Tp::container_type> i = equal_end(fix.container,
                                                                  model),
                                                    j = i;
        BOOST_CHECK(--i != equal_end(fix.container, model));
