...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_basics, Tp, every_quad) {
    Tp                                            fix(0);
    ...
    ordered_iterator_pair<const typename Tp::container_type> r, s(c, c), t(p);
    BOOST_CHECK(a == b);
    BOOST_CHECK(!(a != c));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_dereference, Tp, quad_maps) {
    Tp                                            fix(1, same());
    ordered_iterator<typename Tp::container_type> a(fix.container,
                                                    fix.container.begin());
    ordered_iterator<const typename Tp::container_type> b(
        fix.container, fix.container.begin());
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_minimum, Tp, int2_sets) {
    {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_maximum, Tp, int2_sets) {
    {
        Tp fix(100, randomize(-20, 20));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_increment, Tp, double6_sets) {
    {  // test the invarient of the increment
        Tp                                            fix(20, randomize(-1, 1));
        ordered_iterator<typename Tp::container_type> iter = ordered_begin(
                                                          fix.container),
                                                      end = ordered_end(
                                                          fix.container);
    ...
    {  // test at the limit: a tree where all elements are the same
        ...
        int     count = 0;
        double6 expected;
        std::fill(expected.begin(), expected.end(), 100.0);
        for (; iter != end; ++iter) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_decrement, Tp, double6_sets) {
    {  // test the invarient of the increment
        Tp fix(100, randomize(-1, 1));
        std::reverse_iterator<ordered_iterator<typename Tp::container_type> >
                iter(ordered_end(fix.container)), end(ordered_begin(fix.container));
        int     count = 0;
        double6 tmp   = *iter;
        for (; iter != end; ++iter) {
            BOOST_CHECK(!double6_ordered_less()(tmp, *iter));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_lower_bound, Tp, quad_maps) {
    {  // find the smallest element that is greater or equal to key
        ...
        quad upper(2, 2, 2, 2);
        ordered_iterator<typename Tp::container_type> iter(
            ordered_lower_bound(fix.container, in));
        BOOST_CHECK(iter == ordered_end(fix.container) ||
    ...
    {  // same test with a tree filled with similar values
        ...
        ordered_iterator<typename Tp::container_type> iter(
            ordered_lower_bound(fix.container, lower));
        BOOST_CHECK(iter == ordered_begin(fix.container));
    ...
    {  // test at the limit: tree with 1 value
        ...
        quad                                                upper(2, 2, 2, 2);
        ordered_iterator<const typename Tp::container_type> iter(
            ordered_clower_bound(fix.container, lower));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_upper_bound, Tp, quad_maps) {
    {  // find the smallest element that is greater than key
        ...
        quad upper(1, 1, 1, 1);
        ordered_iterator<typename Tp::container_type> iter(
            ordered_upper_bound(fix.container, in));
        BOOST_CHECK(iter == ordered_end(fix.container) ||
    ...
    {  // same test with a tree filled with similar values
        ...
        quad                                          upper(101, 101, 101, 101);
        ordered_iterator<typename Tp::container_type> iter(
            ordered_upper_bound(fix.container, lower));
        BOOST_CHECK(iter == ordered_begin(fix.container));
    ...
    {  // test at the limit: tree with 1 value
        ...
        quad                                                upper(2, 2, 2, 2);
        ordered_iterator<const typename Tp::container_type> iter(
            ordered_cupper_bound(fix.container, lower));
        BOOST_CHECK(iter == ordered_cbegin(fix.container));
...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_ordered_range, Tp, every_quad) {
    Tp fix(20, randomize(-100, 100));
    {  // non-const
        ordered_iterator_pair<typename Tp::container_type> pair(
            ordered_range(fix.container));
    ...
    {  // const
        ordered_iterator_pair<const typename Tp::container_type> pair0(
            ordered_range(fix.container));  // cast constructor
        BOOST_CHECK(pair0.first == ordered_begin(fix.container));
        BOOST_CHECK(pair0.second == ordered_end(fix.container));
        ordered_iterator_pair<const typename Tp::container_type> pair1(
            ordered_crange(fix.container));  // copy constructor
