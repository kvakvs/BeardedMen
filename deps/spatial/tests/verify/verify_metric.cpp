...
BOOST_AUTO_TEST_CASE(test_difference_bracket) {
    bracket_minus<int2, int> diff =
        details::with_builtin_difference<point_multiset<2, int2> >()(
...
BOOST_AUTO_TEST_CASE(test_difference_paren) {
    ...
    paren_minus<int2, int> diff =
        details::with_builtin_difference<pointset_type>()(pointset_type());
    int2 p(0, 1);
    int2 q(2, 0);
    BOOST_CHECK_EQUAL(diff(0, p, q), -2);
    BOOST_CHECK_EQUAL(diff(1, p, q), 1);
...
BOOST_AUTO_TEST_CASE(test_difference_accessor) {
    typedef point_multiset<4, quad, accessor_less<quad_access, quad> >
        pointset_type;
    accessor_minus<quad_access, quad, int> diff =
        details::with_builtin_difference<pointset_type>()(pointset_type());
    quad p(0, 1, 0, 0);
    quad q(2, 0, 0, 0);
    BOOST_CHECK_EQUAL(diff(0, p, q), -2);
...
BOOST_AUTO_TEST_CASE(test_euclid_distance_to_key) {
    ...
    }
    {
        ...
        for (int i = 0; i < 100; ++i) {
            double6 p = make_double6(drand(), drand(), drand(), drand(),
                                     drand(), drand());
            double6 q = make_double6(drand(), drand(), drand(), drand(),
                                     drand(), drand());
            double dist = math::euclid_distance_to_key<
                double6, bracket_minus<double6, double>, double>(
                6, p, q, bracket_minus<double6, double>());
            using namespace ::std;
            double other_dist = sqrt(
                (p[0] - q[0]) * (p[0] - q[0]) + (p[1] - q[1]) * (p[1] - q[1]) +
                (p[2] - q[2]) * (p[2] - q[2]) + (p[3] - q[3]) * (p[3] - q[3]) +
                (p[4] - q[4]) * (p[4] - q[4]) + (p[5] - q[5]) * (p[5] - q[5]));
    ...
    }
    {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            int dist = math::manhattan_distance_to_key<
                ...
                4, p, q, accessor_minus<quad_access, quad, int>());
            using namespace ::std;
            int other_dist = abs(p.x - q.x) + abs(p.y - q.y) + abs(p.z - q.z) +
                             abs(p.w - q.w);
            BOOST_CHECK_EQUAL(dist, other_dist);
        }
    }
