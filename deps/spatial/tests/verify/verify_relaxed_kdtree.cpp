...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_ctor) {
    typedef details::Relaxed_kdtree<details::Dynamic_rank, int2, int2,
                                    bracket_less<int2>, tight_balancing,
                                    std::allocator<int2> >
                kdtree_type;
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_tight) {
    typedef details::Relaxed_kdtree<details::Static_rank<2>, int2, int2,
                                    bracket_less<int2>, tight_balancing,
                                    std::allocator<int2> >
                                kdtree_type;
    kdtree_type                 tree;
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_perfect) {
    typedef details::Relaxed_kdtree<details::Static_rank<2>, int2, int2,
                                    bracket_less<int2>, perfect_balancing,
                                    std::allocator<int2> >
                                kdtree_type;
    kdtree_type                 tree;
    kdtree_type::const_iterator it;
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_loose) {
    typedef details::Relaxed_kdtree<details::Static_rank<2>, int2, int2,
                                    bracket_less<int2>, loose_balancing,
                                    std::allocator<int2> >
                                kdtree_type;
    kdtree_type                 tree;
    kdtree_type::const_iterator it;
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_growing) {
    ...
    // because the tree is ordered on all dimensions and nodes are all
    // sequential
    // values, iteration should be ordered as well.
    int i = 0;
    for (tight_pointset_fix<int2>::container_type::iterator
             it = fix.container.begin();
         it != fix.container.end(); ++it, ++i) {
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_same) {
    ...
    // values, iteration should be ordered as well.
    int i = 0;
    for (perfect_pointset_fix<int2>::container_type::iterator
             it = fix.container.begin();
         it != fix.container.end(); ++it, ++i) {
        BOOST_CHECK_EQUAL((*it)[0], i);
        BOOST_CHECK_EQUAL((*it)[1], i);
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_reduce) {
    ...
    // values, iteration should be ordered as well.
    int i = 1;
    for (pointset_fix<int2>::container_type::iterator
             it = fix.container.begin();
         it != fix.container.end(); ++it, ++i) {
        BOOST_CHECK_EQUAL((*it)[0], i);
        BOOST_CHECK_EQUAL((*it)[1], i);
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_insert_lots) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        for (pointset_fix<int2>::container_type::iterator it =
                 fix.container.begin();
             it != fix.container.end(); ++it) {
            if ((*it)[0] >= -4 && (*it)[0] < 4 && (*it)[1] >= -4 &&
                (*it)[1] < 4)
                ++count;
        }
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_copy) {
    // Copy a kdtree of quad and iterate over it. Both tree should have leaf
    // nodes
    ...
    for (; orig_begin != orig_end && copy_begin != copy_end;
        ...
        if (orig_begin.node->left == 0)
            BOOST_CHECK(copy_begin.node->left == 0);
        if (orig_begin.node->right == 0)
            BOOST_CHECK(copy_begin.node->right == 0);
    }
}
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_bulk_insert) {
    ...
    for (spatial::import::array<int2, 100>::iterator i = points.begin();
         i != points.end(); ++i) {
        (*i)[0] = std::rand() % 20;
        (*i)[1] = std::rand() % 20;
    }
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_erase_key) {
    ...
    }
    {
        ...
        fix.container.insert(some_value);
        runtime_pointset_fix<int2>::container_type::iterator begin_before =
            fix.container.begin();
        runtime_pointset_fix<int2>::container_type::iterator end_before =
            fix.container.end();
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_erase_iterator) {
    // check that erase at edge preserve basic iterators
    {
        ...
        typedef point_multimap_fix<quad, std::string>::container_type::iterator
                  iter_type;
        ...
        while (fix.container.size() != 0) {
            ...
            int count = 0;
            for (iter_type i = fix.container.begin(); i != fix.container.end();
                 ++i, ++count)
                ;
    ...
    }
    {
        ...
        typedef point_multimap_fix<quad, std::string>::container_type::iterator
            iter_type;
        typedef point_multimap_fix<
            quad, std::string>::container_type::reverse_iterator riter_type;
        size_type track_size = fix.container.size();
        while (fix.container.size() != 0) {
    ...
    // erase all and check that total ordering is preserved.
    {
        ...
        while (fix.container.size() != 0) {
            ...
            std::advance(eraser,
                         static_cast<ptrdiff_t>(static_cast<size_t>(rand()) %
                                                fix.container.size()));
...
BOOST_AUTO_TEST_CASE(test_relaxed_kdtree_lexicographic) {
    {
        ...
        two.container.insert(std::make_pair(quad(2, 2, 1, 1), ""));
        BOOST_CHECK(
            !(one.container < two.container || one.container > two.container));
    }
    {
